document.getElementById('reportForm').addEventListener('submit', function(event) {
    ...
    reportItem.textContent = `Email: ${email}, Título: ${subject}, Descrição: ${description}, Categoria: ${category}, Localização: ${location}`;

    // Se houver mídia, criar link para visualizar
    if (media) {
        const mediaLink = document.createElement('a');
        mediaLink.href = URL.createObjectURL(media);
        mediaLink.target = '_blank';
        mediaLink.textContent = ` (Abrir Mídia: ${media.name})`;
        reportItem.appendChild(mediaLink);
    }
